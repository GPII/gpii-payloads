...
2. [Input Payload](#user-content-input-payload)
3. [Return Payload](#user-content-return-payload)

### Input - User authorized preferences
The user authorized preferences defined by the user `vladimir` for the solution `org.gnome.desktop.interface`:

```
selectedPreferences: { 
    "visual-styling.change-contrast.high-contrast-theme": true
}
```

### Input Payload

The payload received at the OAuth server is identical with [the payload returned by the match maker](CloudBasedMatchMaker.md#user-content-return-payload).
